...
class CnfinCrawler:
    """中国金融信息网爬虫，使用类似FinancialNewsCrawler的结构获取文章"""
...
def save_to_excel(results):
    ...
    # 准备数据
    data = {
        '网站名称': [results['Website_Name']],
        '网站URL': [results['Website_URL']],
        '文章标题': [results['Article_Title']],
        '文章URL': [results['Article_URL']],
        '发布日期': [results['Article_Pub_Date']],
        '爬取时间': [results['Crawl_Time']],
        '文章内容': [results['Article_Content']]
    }
    ...
    # 保存到Excel
    excel_path = "cnfin_latest_article.xlsx"
...
def main():
    ...
    if results:
        print("爬取成功!")
        print(f"标题: {results['Article_Title']}")
        print(f"链接: {results['Article_URL']}")
        print(f"发布日期: {results['Article_Pub_Date']}")
        print(f"内容长度: {len(results['Article_Content'])}")
